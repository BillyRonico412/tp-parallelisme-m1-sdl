...
class Data_item:
    ...
    def __str__(self):
        return "ID=" + str(self.id) + " POS=(" + str(self.positionx) + "," + str(self.positiony) + ") SPEED=(" + str(
            self.speedx) + "," + str(self.speedy) + ") WEIGHT=" + str(self.weight)
...
def interaction(i, j):
    dist = math.sqrt((j.positionx - i.positionx) * (j.positionx - i.positionx) + (j.positiony - i.positiony) * (
            j.positiony - i.positiony))
    if i == j:
        return (0, 0)
    g = 6.673e-11
...
def init_world(n):
    data = [Data_item(id=i, positionx=1e18 * math.exp(-1.8) * (.5 - random.uniform(0, 1)),
                      positiony=1e18 * math.exp(-1.8) * (.5 - random.uniform(0, 1)), speedx=0, speedy=0,
                      weight=(random.uniform(0, 1) * solarmass * 10 + 1e20)) for i in range(n - 1)]
...
b_inf = (nbbodies // size) * rank
b_sup = (nbbodies // size) * (rank + 1)
...
for t in range(NBSTEPS):
    ...
    for i in range(b_inf, b_sup):
        force.append((0, 0))
        for j in range(nbbodies):
            inter = interaction(data[i], data[j])
            force[i - b_inf] = (force[i - b_inf][0] + inter[0], force[i - b_inf][1] + inter[1])

    for i in range(b_inf, b_sup):
        data[i] = update(data[i], force[i - b_inf])

    data = comm.allreduce(data[b_inf: b_sup], op=MPI.SUM)

    if rank == 0:
        displayPlot(data)

sign = signature(world=data)
