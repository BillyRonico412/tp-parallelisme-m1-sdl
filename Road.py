...
import time
import random
from multiprocessing import Process, Value, Lock, Condition, Array


class Road:
    def __init__(self):

        # Type circulant sur la voie -1, 0 ou 1
        self.type_VU = Value('i', -1)

        # Nombre de processus en attente
        self.nb_attente_v = Array('i', [0, 0])

        # Nombre vehicule qui passe
        self.nb_sur_voie = Value('i', 0)

        # Condition et mutex
        self.mutex = Lock()
        self.cond_v = (Condition(self.mutex), Condition(self.mutex))

    def enter_road(self, direction):
        with self.mutex:
            while self.type_VU.value != direction and self.type_VU.value == -1:
                self.cond_v[direction].wait()

            if self.type_VU == -1:
                self.type_VU += 1
            self.nb_sur_voie.value += 1
            self.cond_v[direction].notify()

    def exit_road(self):
        with self.mutex:
            self.nb_sur_voie.value -= 1
            if self.nb_sur_voie.value == 0:
                dir = self.type_VU.value
                self.type_VU.value = -1
                self.cond_v[(dir + 1) % 2].notify()
